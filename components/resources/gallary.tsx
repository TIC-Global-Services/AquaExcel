    ...
    <div className="w-full max-w-[1440px] mx-auto py-10">
      ...
      <div className="md:hidden flex overflow-x-auto snap-x snap-mandatory gap-3 pb-0 scrollbar-hide">
        {GALLERY_ITEMS.map((item) => (
          <div
            key={item.id}
            className="relative flex-shrink-0 w-[75vw] h-[300px] rounded-[20px] overflow-hidden snap-center"
          >
            <Image
              src={item.image}
              alt={item.title}
      ...
      {/* Desktop: Grid Layout */}
      <div className="hidden md:grid grid-cols-2 lg:grid-cols-6 gap-4 lg:gap-6">
        {GALLERY_ITEMS.map((item) => (
